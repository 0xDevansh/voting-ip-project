...
class ElectionNavigationFrame(ttk.Frame):
    def __init__(self, app, context):
        ...
        for i in range(len((text_for_labels))):
            Labels.append(tk.Label(election_nav_frame_1,text=text_for_labels[i]))
            Labels[i].grid(row = 0 , column= i)
        election_nav_frame_2=tk.LabelFrame(self.frame)
        election_nav_frame_2.grid(row=1, column=0, sticky="news")
        Ref_name_label=[]
        Ref_status_Label=[]
        ...
        for i in range(len(Election_Status_List)):
            ...
            Ref_name_label[i].grid(row=i, column=0)
            Ref_status_Label[i].grid(row=i, column=1, padx=50, pady=10)
            ...
            if Election_Status_List[i] == 'Not_Started':
                 ...
                 Ref_command_button_1[i].grid(row=i, column=2, padx=50, pady=10)
