...
class ReferendumEntryFrame(ttk.Frame):
    def __init__(self, app, context):
        ...
        for i in range(len((text_for_labels))):
            Labels.append(ttk.Label(ref_entry_frame, text=text_for_labels[i]))
            Labels[i].grid(row=0, column=i, padx=50)


        for i in range(num_proposals):
           l1.append(ttk.Label(ref_entry_frame, text="Proposal" + str(i + 1)))
           l2.append(ttk.Entry(ref_entry_frame))
           l3.append(ttk.Entry(ref_entry_frame))
        ...
        button = ttk.Button(ref_entry_frame, text="Submit" , command= get_data)
